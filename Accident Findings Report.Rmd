...
exposure <- rr_crossing_data_gr %>% 
  ...
  select(Longitude, Latitude, Street, "Gate_Arm_N" = `Count Roadway Gate Arms`, Radius_Count, `Total Daylight Thru Trains`, `Total Nighttime Thru Trains`) %>% 
...
```{r}
exposure %>% 
  filter(Gate_Arm_N > 0, (`Total Daylight Thru Trains` > 0 | `Total Nighttime Thru Trains` > 0)) %>% 
  nrow()
```
